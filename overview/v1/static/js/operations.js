...
function projectAgentsNowText(project) {
  ...
  if(!seats.length) return 'none staffed';
  return seats.map(a=>{
    const name=a.id.replace(/^bp-/,'').replace(/-implementer$/,'');
    const badge=a.state==='working' ? 'working' : a.badge.toLowerCase();
    return `${name} · ${badge}`;
  }).join(', ');
}
...
function projectGoLinks(project) {
  ...
  wrap.append(link('Work','/work?'+params(project.id)), link('For You','/work?'+new URLSearchParams({project:project.id,attention:'any'})), link('Papers','/documents?'+params(project.id)), link('Map','/map?project='+encodeURIComponent(project.id)));
  return wrap;
}
...
function projectComparisonRow(project) {
  ...
  row.append(name, projectCountCell(project,'open'), projectCountCell(project,'attention'),
    project.state==='available' ? el('span',String(project.deferred ?? 0)) : el('span','—','bp-muted'),
...
function projectIsQuiet(project) {
  ...
  const seats=(snapshot.agents || []).filter(a=>a.group==='seat' && a.project===project.id);
  return !project.open && !project.running && !project.claimed && !project.attention && !seats.length;
}
