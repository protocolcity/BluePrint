...
Use the canonical build, activation, verification, and recovery procedure in [DEPLOYMENT.md](../../docs/operations/DEPLOYMENT.md).

Run `blueprint status --root /path/to/workspace` to identify the responding build. Development uses `blueprint serve --foreground --root /path/to/workspace --port 8893`; stop the preview after verification. Production activation normally serves localhost:8801. Leftover split ports :8802 and :8803 redirect. Do not create additional persistent UI services or use a second Homebrew update path.
