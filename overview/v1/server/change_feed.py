...
def _signature(paths: Iterable[Path]) -> tuple:
    ...
    Covers appends (mtime/size move), additions and removals (the tuple's
    membership changes) without needing to know the file list ahead of
    time.
    """
    rows = [row for row in (_stamp(p) for p in paths) if row is not None]
...
def _changed_name(before: tuple, after: tuple) -> str:
    ...
    for row in after:
        if prior.get(row[0]) != row:
            return row[0]
    removed = set(prior) - {row[0] for row in after}
    return sorted(removed)[0] if removed else ""


def _worklane_paths(root: Path) -> list[Path]:
    ...
    if not data.is_dir():
        return []
    return list(data.glob("*.db"))


def _workforce_paths(root: Path) -> list[Path]:
    paths: list[Path] = []
    ...
    if daemon is not None:
        ...
        if ledger.is_dir():
            paths.extend(p for p in ledger.iterdir() if p.is_file())
    return paths


def _supervisor_paths(root: Path) -> list[Path]:
    roster = resolve_roster_path(root)
    if roster is None:
        return []
    ...
    if not reports.is_dir():
        return []
    return [p for p in reports.iterdir() if p.is_file()]
