...
| Checkout on the Mini | **`example_user/ProtocolCity-BluePrint`** |
| Isolated agent HOME | **`~/.bots/blueprint`** — prefer it **when present** |
| Workspace / binder | **`example_user`** |

**Rule:** if `~/.bots/blueprint` exists, run the session under that isolated HOME.
Otherwise just open the checkout at `example_user/ProtocolCity-BluePrint` directly.
...
| **:8803** | `blueprint-overview` | Four-lens desk. **MUST keep `--binder example_user`.** |
...
`example_user/AGENTS.md` is **consume-only operator law** — it governs how the
...
cd example_user/ProtocolCity-BluePrint
...
python3 overview/v1/serve.py --port 8803 --binder example_user
...
python3 map/v1/serve.py --binder example_user --port 8802
...
```bash
blueprint-overview --binder example_user --port 8803
blueprint-map      --binder example_user --port 8802
```
