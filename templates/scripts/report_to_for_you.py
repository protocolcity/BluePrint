...
**human-gated** work order tagged to You so Map · You · FOR YOU lists it.
  ...
  python3 scripts/report_to_for_you.py \\
    ...
    --path tradeOS/local/reports/maru/2026-08-02-desk-brief.md \\
    --workspace example_user

  # scan common report slots and drop missing inbox items for today
  python3 scripts/report_to_for_you.py --scan --workspace example_user

  # per-product efficiency stays disk-only unless act-now smell (pc-912)
  python3 scripts/report_to_for_you.py --scan --workspace example_user --act-now
...
**Efficiency policy (FOR_YOU_INBOX_REPORTS / DAILY_REPORTS_MAP / pc-912):**
engine ``efficiency-*``, tradeOS ``efficiency-pass``, and ``suite-efficiency``
...
**Dual-audience (FOR_YOU_INBOX_REPORTS §Dual-audience / pc-916 / pc-939):**
...
**One product gold / day (pc-916 / pc-941):** tradeOS desk brief + HTML
glance + RSU pack are **one** human-gated card (key ``desk-brief``, same
idempotency label as tradeOS ``for_you_drop``). ``--scan`` does not mint a
...
# tradeOS product gold is ``desk-brief`` only (pc-941); RSU folds in.
...
def extract_dual_sections(text: str) -> Tuple[Optional[str], Optional[str]]:
    """Pull Builder / User bodies from report markdown (pc-939).
...
def format_dual_description(
    *,
    ...
    Matches FOR_YOU_INBOX_REPORTS §Report-body structure (pc-916 / pc-939).
...
def canonical_report_key(key: str) -> str:
    """Map legacy / secondary keys to the stable drop key (pc-941)."""
    k = _slug_key(key)
...
def is_folded_into_desk_key(key: str) -> bool:
    """True when this key must not mint a separate gold (pc-941)."""
...
def is_thin_report(
    ...
    *,
    ...
    """True when a product report is too thin for its own gold (pc-916).
...
def is_disk_only_efficiency_key(key: str) -> bool:
    """True when this report key must not mint routine For You gold (pc-912).
...
def drop_report(
    *,
    ...
    # pc-941: legacy maru-desk-brief / rsu-window → one desk-brief card
    ...
    # Dual-audience body (pc-939) — dig-in still uses REPORT face; **Report:**
...
def _rsu_candidates(day: str, day_utc: str) -> List[str]:
    ...
    return [
        "tradeOS/local/reports/maru/%s-rsu-window.md" % month,
        "tradeOS/local/reports/maru/%s-rsu-window.md" % month_utc,
        "tradeOS/local/reports/maru/%s-rsu-window.md" % day,
        "tradeOS/local/reports/maru/%s-rsu-window.md" % day_utc,
        "tradeOS/local/reports/maru/2026-08-rsu-window.md",
    ]
...
def scan_and_drop(
    ...
    *,
    ...
    (pc-912). Workspace ``workspace-efficiency`` remains the single rollup gold.

    Thin / all-ops product files skip per-product gold (pc-916 / pc-939) and
    fold into one workspace ``workspace-thin-rollup`` card when any exist.

    tradeOS product gold is one ``desk-brief`` card (pc-941) — same key as
    ...
    # tradeOS: one desk-brief slot (not maru + rsu) — pc-941.
    slots: List[Tuple[str, str, str, List[str], Optional[str]]] = [
        (
            "tradeos",
            "desk-brief",
            "tradeOS · desk brief · %s" % day,
            [
                "tradeOS/local/reports/maru/%s-desk-brief.md" % day,
                "tradeOS/local/reports/maru/%s-desk-brief.md" % day_utc,
            ],
            "tradeOS/local/reports/for-you/latest.html",
        ),
        # rsu-window listed only so scan can report folded/skip (not gold)
        (
            "tradeos",
            "rsu-window",
            "tradeOS · RSU window pack",
        ...
        (
            "tradeos",
            "efficiency-pass",
            "tradeOS · efficiency pass · %s" % day,
            [
                "tradeOS/local/reports/efficiency-pass/%s.md" % day,
                "tradeOS/local/reports/efficiency-pass/%s.md" % day_utc,
            ],
            None,
        ),
        (
            "tradeos",
            "board-validation",
            "tradeOS · board validation · %s" % day,
            [
                "tradeOS/local/reports/board-validation-%s.md" % day,
                "tradeOS/local/reports/board-validation-%s.md" % day_utc,
            ],
            None,
        ),
        (
    ...
    # Resolve tradeOS desk brief + RSU once so we can fold (pc-941)
    ...
    for project, key, title, cands, visual in slots:
        ...
        if not path:
            ...
            continue
        # pc-912: never mint N× per-product efficiency (or board-validation) gold
        if is_disk_only_scan_key(key) and not act_now:
            ...
            continue
        # pc-941: RSU never mints its own gold — fold into desk-brief
        if is_folded_into_desk_key(key):
            ...
            if tradeos_desk_dropped:
                ...
                continue
            key = "desk-brief"
            title = "tradeOS · desk brief · %s" % day
            # fall through to drop as desk-brief
        # pc-916 / pc-939: thin product output → workspace rollup, not gold spam
        ...
        if project == "tradeos" and canonical_report_key(key) == "desk-brief":
            tradeos_desk_dropped = True
        results.append(r)
    ...
    if thin_hits:
        ...
        lines = [
            ...
            "minting one gold each (FOR_YOU dual-audience / pc-916).\n\n",
            "## Builder\n\n",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "(act-now smell only; default is disk-only — pc-912)"
        ),
    )
    ...
    # (pc-912). Also refuse standalone rsu-window gold (pc-941 — fold to desk).
    ...
    if is_folded_into_desk_key(args.key) and not args.act_now:
        r = {
            ...
            "key": args.key,
            "reason": (
                "rsu-window folds into the single tradeOS desk-brief gold "
                "(pc-941); drop with --key desk-brief (and RSU as related "
