...
2. **Probe `queue_url`** on the roster row — must be
   ...
   or wrong URL → fix roster or file WorkForce process WO (wf-109 class).
...
| Tool | Bridge |
|---|---|
| Grok | `~/.grok/config.toml` → `[skills] paths = ["example_user/.agents/skills"]` |
...
| Hard-down pool, no For You gold | `python3 -m workforce capacity --live` (wf-120) or `report_to_for_you.py --project workforce --key capacity-<pool> --path <report>` |
...
- [ ] Hard-down pool today (weekly 0% / usage 0%): check For You for an
  ...
  ```bash
  python3 -m workforce capacity --live   # wf-120 (wired to cadence by wf-122)
  # manual fallback:
...
- Capacity thrash events noted; hard-down pools have a For You gold (or wf-120/wf-122 queued)
